...
const {
  ...
  Card, CardContent, Grid, LinearProgress, IconButton, Tooltip, Divider,
  TablePagination, Snackbar, Stack, FormControl, InputLabel, Select, MenuItem
} = MaterialUI;
const { CloudUpload, Download, CheckCircle, Error, BarChart, Refresh, Delete } = MaterialUIIcons;

const API_BASE = 'http://localhost:8080/api/gst';
  ...
  return (
    <Paper
      ...
      sx={{
        p: 3,
        mb: 3,
        border: dragActive ? `2px dashed ${theme.primary}` : '2px dashed #ccc',
    ...
    >
      <Box textAlign="center" py={3}>
        <CloudUpload sx={{ fontSize: 64, color: theme.primary, mb: 2 }} />
  ...
  return (
    <Grid container spacing={3} mb={3}>
      <Grid item xs={12} sm={6} md={2.4}>
        <StatCard title="Total Records" value={stats.totalRecords} color={theme.primary} icon="📊" />
      </Grid>
      <Grid item xs={12} sm={6} md={2.4}>
        <StatCard title="Valid GST" value={stats.validCount} color={theme.success} icon="✓" />
      </Grid>
      <Grid item xs={12} sm={6} md={2.4}>
        <StatCard title="Invalid GST" value={stats.invalidCount} color={theme.error} icon="✗" />
      </Grid>
      <Grid item xs={12} sm={6} md={2.4}>
        <StatCard title="Tamil Nadu" value={stats.tamilNaduCount} color={theme.warning} icon="🏛" />
      </Grid>
      <Grid item xs={12} sm={6} md={2.4}>
        <StatCard title="Puducherry" value={stats.puducherryCount} color="#9c27b0" icon="🏛" />
      </Grid>
    </Grid>
  );
}
    ...
    <Box sx={{ bgcolor: theme.background, minHeight: '100vh', py: 4 }}>
      <Container maxWidth="xl">
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={4}>
          <Box>
            <Typography variant="h4" component="h1" fontWeight={700} color={theme.primary}>
              Bulk GST Validator
            </Typography>
